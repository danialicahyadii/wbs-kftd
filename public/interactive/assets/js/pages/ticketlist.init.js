...
(xhttp.onload = function () {
    ...
    // xhttp.open("GET", "interactive/assets/json/support-tickets-list.json"),
    xhttp.send();
...
function SearchData() {
    ...
    ticketsList.filter(function (e) {
        var t = (matchData = new DOMParser().parseFromString(
            ...
            i = "all" == t || "all" == s || t == s,
            a =
                new Date(e.values().create_date.slice(0, 12)) >= new Date(n) &&
                new Date(e.values().create_date.slice(0, 12)) <= new Date(d);
        return (i && a) || (i && "" == l ? i : a && "" == l ? a : void 0);
    }),
        ticketsList.update();
}
var count = 14,
                        ...
                        ? (e.preventDefault(),
                          ...
                          editlist
                              ? "" !== tasksTitleField.value &&
                                ...
                                "" !== priorityField.value &&
                                editlist &&
                                ((t = ticketsList.get({ id: idField.value })),
...
function ischeckboxcheck() {
    Array.from(document.getElementsByName("checkAll")).forEach(function (i) {
...
function ViewTickets(e) {
    var e = e.getAttribute("data-id"),
        ...
        t = ((i.innerHTML = t), i.innerText.slice(4));
    localStorage.setItem("ticket-list", JSON.stringify(e[0]._values)),
        ...
        window.location.assign(route("pengaduan.show"));
}
document
    .querySelector(".pagination-next")
    .addEventListener("click", function () {
